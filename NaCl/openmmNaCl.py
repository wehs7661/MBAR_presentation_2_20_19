...
# First we 'bin' (count the total number of samples) in each thermodynamic state
 ...
 print("Binning the samples using "+str(num_states)+" temperature windows of "+str(T_step_size)+" K.")
 state_ranges = np.array([[T_min+(i*T_step_size),T_min+((i+1)*T_step_size)] for i in range(0,num_states)])
 T_state_center = sum(state_ranges[window_index])/2.0
 state_counts = np.array([0 for i in num_states])
 U_reduced_inter_state = np.array([[U_uncorrelated[index]/T_state_center for index in U_uncorrelated] for state in num_states]) # Equivalent to u_kn in mbar.py
 for T in T_uncorrelated:
  state_index = 0
  for state in state_ranges:
   if T >= state[0] and sample_T < state[1]:
    state_counts[window_index] = state_counts[window_index] + 1
